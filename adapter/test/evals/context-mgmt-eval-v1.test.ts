// Spec: ../../../workspaces/context-mgmt-eval-v1/eval.spec.md
//
...
interface Scenario {
  /** Adapter-side skill id; the eval addresses this in the OpenAI body's `model` field. */
  skillId: string;
...
const SCENARIOS: Scenario[] = [
  {
    skillId: "context-mgmt-eval-v1",
    label: "configured workspace",
    expectation: "present",
  },
  {
    skillId: "context-mgmt-eval-bare-v1",
...
for (const scenario of SCENARIOS) {
  describe(`${scenario.skillId} — ${scenario.label} (live adapter evals)`, () => {
    before(async () => {
    ...
    it(
      ...
      async () => {
        ...
        assertContains(content, marker, scenario.expectation, `A1/${scenario.skillId}`);
      },
    );
    ...
    it(
      ...
      async () => {
        ...
        assertContains(content, marker, scenario.expectation, `B1/${scenario.skillId}`);
      },
    );
    ...
    it(
      ...
      async () => {
        ...
        assertContains(content, `NONCE=${nonce}`, scenario.expectation, `C1/${scenario.skillId}`);
        ...
        if (scenario.expectation === "present") {
          assert.match(
            ...
            `[C1/${scenario.skillId}] expected URL line with an oreilly.com URL; got: ${preview(content)}`,
          );
        }
      },
    );
